      ...
      <nav className={`flex items-center justify-between h-[3.85rem] text-content lg:pl-16 lg:pr-16 pl-4 pr-4 sticky top-0 z-50 ${scrolling ? "bg-bgDark" : " bg-bgDark"}`}>
        <Link to="/" onClick={() => handleLinkClick(1)} className="text-2xl font-bold  tracking-widest">
          <div className="flex gap-1 items-center">
            <span className="absolute md:left-32 top-4 left-16 text-amber-400 z-[999] ml-3 md:ml-0 size-20" >PedalConnect</span>
          </div>
        </Link>
        ...
        <div className="md:hidden absolute w-full block bg-bgDark left-0 top-[3.75rem] z-[5]">
          {open && (
            <div className="h-[calc(100vh-5.75rem)] flex flex-col justify-center items-center gap-4">
              {navigation.map((nav) => (
                <Link to={nav.src} spy={true} smooth={true} offset={-70}  duration={500} onClick={() => handleLinkClick(nav.id)} className={`flex items-center justify-center ml-4 mr-4 p-2 w-52 gap-1 cursor-pointer text-lg font-medium rounded-lg hover:-translate-y hover:bg-amber-400 hover:text-bgDark hover:scale-110 ${nav.current ? "bg-amber-400 text-bgDark" : ""}`} key={nav.id}>
                  {nav.icon()}
                  {nav.name}
                </Link>
              ))}
